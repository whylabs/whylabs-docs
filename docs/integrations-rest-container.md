...
We offer multiple solutions to transporting dataset profiles into WhyLabs in
order to accommodate the width breadth of environments and pipelines that
exist today. One of these solutions is a container that starts a rest
endpoint that takes in data in pandas format and runs whylogs on that data
for you, periodically uploading the dataset profiles it creates to WhyLabs.
It is intended to be run on customer premises.
...
configured mostly via environment variables and then run through Docker. The
...
If you're working in python with pandas then you can get the right format directly out of the pandas DataFrame.
